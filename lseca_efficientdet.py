...
def lse_pool(x, dim, tau=1.0):
    ...
    out = tau * (
        torch.logsumexp(x32 / tau, dim=dim, keepdim=True)
        - math.log(n)
    )
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32, tau=1.0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # LSE directional pooling
        x_h = lse_pool(x, dim=3, tau=self.tau)
        x_w = lse_pool(x, dim=2, tau=self.tau)
...
class MBConv(nn.Module):
    def __init__(
        ...
        # CA
        self.ca = CoordAtt(mid_channels, mid_channels)
        # SE
